...
export default async function Page({ searchParams }) {
    ...
    let data        = await getPostsByFilterType('search', searchParams.s, currentPage, 10);
    ...
    const renderPagination = () => {
        ...
        for (let i = 0; i < visiblePages.length; i++) {
          ...
          paginationElements.push(
            ...
            >
              <Link href={`/search?s=${searchParams.s}&page=${page}`}>{page}</Link>
            </li>
          );
        }
    ...
    return (
        <>
            <Breadcrumb type={'search'} name={searchParams.s}></Breadcrumb>
                                ...
                                <div className="pagination__wrap">
                                    <ul className="list-wrap justify-content-center">
                                        <li className={currentPage == 1 ? 'd-none' : ''}>
                                            <Link href={`/search?s=${searchParams.s}&page=1`}><i className="fas fa-angle-double-left"></i></Link>
                                        </li>
                                        {renderPagination()}
                                        <li className={currentPage == totalPages ? 'd-none' : ''}>
                                            <Link href={`/search?s=${searchParams.s}&page=${totalPages}`}><i className="fas fa-angle-double-right"></i></Link>
                                        </li>
                                    </ul>
                                </div>
                            </div>
